...
- **Bulk tracking refresh vs. folded cards** — every order card now starts
  ...
  **RESOLVED (2026-09-08, owner-requested):** neither — the button is gone.
  ...
  night. Written, but NOT deployed yet. See the 2026-09-08 entry at the end of
  this file.
...
## Completed (this session, 2026-09-08) — orders: manual refresh-all replaced by a nightly server-side auto-update
...
`refreshAllTracking`, the `refreshAllLabel` progress state, the
`anyTracked`/`openTracked` derived lists, `clearFold` (it folded each card back
after the batch), and `scrollAllSteppersToCurrent` + its only helper
`centerStepper`. The per-order 🔄 button inside each card is untouched — a
single parcel can still be refreshed by hand at any time.

**The schedule lives in `trinkl/functions`, not here.** First cut was a
panel-side hook (`useDailyTrackingSync`); the owner was shown that it could only
ever run while the admin panel was open and chose the server-side job instead,
so the hook was deleted in the same session and never shipped. `ghost` keeps no
scheduling code at all: the new `refreshAllParcels` scheduled Cloud Function
...
a local function in `orders-view.tsx` to an export in
`components/admin/carriers.ts`. It is still used by the per-order refresh; the
export outlived the hook that was the second caller, and is worth keeping there
as carrier-domain logic rather than view-local.
