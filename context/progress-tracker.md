...
## Quiz intro — the scroll drives itself (2026-09-09)

The intro's scroll-scrubbed video asked every visitor to scroll ~2600px before
the CTA appeared. That is the whole experience, and it was gated behind work.
It now offers to do the scrolling itself.

**The button.** A second button on the opening screen, inside the headline
card: «اكتشفي المنتج المناسب لكِ». Pressing it does not skip the intro — it
plays it. The video plays at `TOUR_RATE` (0.55, so the 10s clip becomes ~18s of
reveal) and every frame the page is scrolled to wherever that moment of the
clip sits in the intro's travel. It rides the headline's own fade and is gone
by the time the second line arrives, so the screen still ends on one CTA.

**The direction of the arrow reverses, and only one may hold it.** Normally
scroll is the input and `video.currentTime` the output. While the tour drives
it is the exact reverse. The scrub effect skips its seek whenever the tour is
driving; without that the two spend every frame overwriting each other and the
picture judders. `tourRef` (not the `tourOn` state) is what it reads, because
that effect is registered once per stage and would close over a stale value.

**Her scroll is the volume knob, not an interruption.** Any wheel, touchmove or
scroll key marks the moment; the tour pauses the clip and yields for
`TOUR_RESUME_MS` (900ms), during which her scrolling seeks the video as it
always did; then it re-anchors on the frame she left it at and carries on.
Nothing snaps back to where the tour would otherwise have got to — including
when she scrolls *backwards*, which is the case that makes it a control rather
than a skip button. Scrolling is forward-only on the tour's side (`target` is
clamped to at least the current position), so being ahead of it is never
punished.

Two inputs fire no event of their own — a scrollbar drag, and touch momentum
after the finger is gone. Those are caught by drift: a scroll position more
than `TOUR_DRIFT_PX` (24px) off the one the tour last asked for is hers. 24 sits
well above the tour's own per-frame step (~3px) and well below one wheel notch
(~100px).

**A dead button is worse than no button.** Headless Chromium has no H.264
decoder, which is how the first version was caught doing nothing at all: with
no `duration` there was no clock, and the press was a no-op. That is not only a
test-harness state — it is a slow connection, and a slow connection is exactly
where scrolling for a minute appeals least. So the tour falls back to a wall
clock over `TOUR_FALLBACK_MS` (18000ms — the same pace, so the copy still gets
its time on screen) and hands over to the video mid-tour if one shows up,
seeking the clip to the frame already on screen before letting it lead.

`window.scrollTo` passes `behavior: "instant"`: `app/globals.css` sets
`scroll-behavior: smooth` on `<html>`, and a smooth scroll re-animating from
every frame's `scrollTo` never arrives anywhere.

Reduced motion hides the button entirely (`.startWrap { display: none }`) —
that mode skips the scrub, so there is no scroll for a tour to drive and the
real CTA is already on screen from the first paint.

**Verified.** Typecheck and `next build` clean; ESLint unchanged (the same 3
pre-existing problems elsewhere). Driven in Chromium at 390px and 1280px, with
a stubbed media clock on the `<video>` to exercise the branch the sandbox
cannot decode: the page scrolls itself with no input and never backwards; a
flick pauses the clip, seeks it, and the tour resumes from *her* frame 900ms
later; scrolling back up is likewise resumed from, not undone; untouched it
runs to `--intro-progress` 1.0, stops, pauses the video and holds still, with
the real CTA clickable; that CTA still starts question 1. The undecodable-video
fallback was driven on the real asset in the same browser. Reduced motion
renders no start button and a live CTA.
